...
if reference:
    if N_source*N_target <= 10000**2:
        ...
        try:
            ...
            self_reference_eval = out['source']['u'].real
            ...
            st = time.time()
            out = pyfmmlib2d.HFMM(source, target, charge=tau, compute_target_potential=True, precision=reference_precision, helmholtz_parameter=1j*helmholtz_k)
            target_reference_eval = out['target']['u'].real
            tt = time.time() - st - tform
...
FMM = KI_FMM(px, py, Modified_Helmholtz_Eval, N_cutoff, Nequiv)
...
st = time.time()
print('')
FMM = KI_FMM(px, py, Modified_Helmholtz_Eval, N_cutoff, Nequiv, bbox=bbox, helper=FMM.helper)
